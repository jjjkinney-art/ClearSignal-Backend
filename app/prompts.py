...
def synthesizer_prompt(
    ...
    return (
        ...
        "  Sentence 4 — Guidance (fluid and human, not a command list):\n"
        "    Tell the user what to do next. Choose the opener that best fits final_verdict:\n"
        f"      bullish      → 'Given this setup, ...'         (forward-looking, action-oriented)\n"
        f"      constructive → 'At this point, it makes sense to ...'  (measured, add-on-weakness tone)\n"
        f"      neutral      → 'If you already own it, ...'    (ownership-focused, patient hold)\n"
        f"      cautious     → 'For now, the best approach is to ...'  (protective, limit-exposure tone)\n"
        f"      bearish      → 'At this point, ...' or 'Given this setup, ...'  (decisive, avoid/exit tone)\n"
        "    Do NOT use these openers for the wrong verdict — the opener must match the stance.\n"
        "    After the opener: be concrete. Hold, wait for a specific event, add on a pullback,\n"
        "    reduce if X happens. Reference one item from what_to_monitor where it fits naturally.\n"
        "    The sentence should feel like friendly advice, not a checklist item.\n\n"
        "  TONE:\n"
        ...
        "  ✗ 'portfolio manager'\n"
        "  ✗ 'risk/reward'\n\n"
        "  EXAMPLES — note how the opener changes with the verdict:\n\n"
        "  Neutral verdict:\n"
        ...
        "   strength, which limits how much upside you're likely to see near term. If you already own it,\n"
        "   it makes sense to hold — just keep an eye on the next earnings call for any sign that\n"
        ...
        "   EV competition are pressuring profitability in the near term. For now, the best approach\n"
        "   is to hold any existing position lightly and avoid adding until gross margins stabilise\n"
        "   above 18% for at least two consecutive quarters.'\n\n"
