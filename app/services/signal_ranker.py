...
def build_confidence_reasoning(
    ...
    ---------------
    GOOD:
      "Confidence is reduced because macro and risk analysis register lower
       conviction versus valuation analysis, indicating disagreement on the
       rate-sensitivity mechanism.  Signal direction is split (3 constructive
       vs 4 cautious) — the thesis does not have clear directional momentum."

    GOOD:
      "Evidence base is solid.  Conviction is high — valuation, macro, and
       risk analysis all point in the same direction, leaving limited room
       for analytical disagreement.  Signal direction is predominantly
       cautious across ranked signals."
    ...
    if conf_range >= 0.30 and low_agents:
        ...
        parts.append(
            f"Conviction is high — {high_str} analysis all point in the same "
            f"direction, leaving limited room for analytical disagreement."
        )
    ...
    if total_n >= 3:
        ...
        if dir_share >= 0.75:
            ...
            parts.append(
                f"Signal direction is split ({bullish_n} constructive vs "
                f"{bearish_n} cautious) — the thesis does not have clear "
