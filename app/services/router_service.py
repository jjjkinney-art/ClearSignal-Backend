...
def _run_investment_pipeline(
    ...
    memory_context_block  : [Phase 9C] Formatted memory block for synthesis prompt injection.
    memory_context_data   : [Phase 9C] Structured memory dict for API response stamping.
    ...
    if memory_context_data:
        try:
            from ..schemas import MemoryContextSummary as _MemCtxSummary
            thesis.memory_context = _MemCtxSummary(**memory_context_data)
        except Exception as _mc_exc:
            logger.debug("[router] 9C memory_context stamp failed (non-fatal): %r", _mc_exc)
